...
IF OBJECT_ID('dbo.sp_GetPaymentByRequestId', 'P') IS NOT NULL
    DROP PROCEDURE dbo.sp_GetPaymentByRequestId;
GO

CREATE PROCEDURE dbo.sp_GetPaymentByRequestId
    @RequestId UNIQUEIDENTIFIER
AS
BEGIN
    SET NOCOUNT ON;

    SELECT * FROM Payments WHERE RequestId = @RequestId;
END;
GO
...
BEGIN
    ...
    DELETE FROM Payments WHERE RequestId = @RequestId;
